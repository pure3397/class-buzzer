...
브라우저에서 아래 주소를 엽니다. 선생님이 교사 화면을 열면 고유한 6자리 방 코드와 학생용 QR이 자동으로 만들어집니다.

- 교사 화면: `http://localhost:3000/?role=teacher`
- 팀 화면: 교사 화면의 QR을 스캔하거나 `http://localhost:3000/?role=student`에서 방 코드를 입력합니다.
...
- 선생님별 독립된 방과 고유 코드·QR 생성
- 방별 참가자·버저·점수 분리, 교사용 조작 키 확인
- 팀전/개인전 전환
- 팀명 직접 입력 후 교사 승인
- 전체 선택, 선택 승인, 선택 거절
...
각 선생님은 `?role=teacher` 주소에서 자기 방을 만듭니다. 교사용 키는 해당 브라우저에 저장되므로 새로고침해도 같은 방으로 돌아옵니다. 다른 기기나 브라우저에서 기존 방을 조작할 수는 없습니다. 새 수업에는 교사 화면 아래쪽의 **새 방 만들기**를 사용하세요. 이전 방은 같은 브라우저에서 기존 교사용 주소를 다시 열면 돌아갈 수 있습니다.

현재 버전은 인메모리 방식입니다. 서버가 재시작되면 **모든 방과 참가 팀·점수가 초기화**되고 기존 QR과 방 코드는 무효가 됩니다. 수업 종료 후에는 결과 화면에서 CSV 또는 PDF로 저장하세요. 재시작 뒤 선생님 화면을 다시 열면 새 방이 생성됩니다.
